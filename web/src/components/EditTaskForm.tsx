...
export function EditTaskForm({ open, onOpenChange, task }: EditTaskFormProps) {
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      {open ? (
        <EditTaskFormContent key={`${task.id}-${open}`} onOpenChange={onOpenChange} task={task} />
      ) : null}
    </Dialog.Root>
  )
}
...
function EditTaskFormContent({ onOpenChange, task }: EditTaskFormContentProps) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            >
              ...
              <option value="overcompleted">Overcompleted</option>
