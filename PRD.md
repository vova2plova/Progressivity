...
status        string, not null               — pending / in_progress / completed / overcompleted / canceled
created_at    timestamptz
updated_at    timestamptz
```
...
value         decimal, not null              — ненулевая инкрементальная прибавка (может быть отрицательной)
note          text (nullable)
...
2. Leaf-задача с `target_value`: прогресс = `sum(entries.value) / target_value`, может быть отрицательным и больше 100%.
...
7. `value` в ProgressEntry — ненулевая инкрементальная прибавка, не абсолютное значение.
8. Для leaf-задач с `target_value`: `progress > 100%` означает перевыполнение плана (`overcompleted`).
...
calculateProgress(task):
  if task has no children:
    if task.target_value != NULL:
      ...
      return 1.0 if task.status in [completed, overcompleted] else 0.0
...
- `completed_children` — количество дочерних задач, достигших или превысивших 100%
